...
function App() {
  ...
  const [activeTab, setActiveTab] = useState(() => {
    ...
    if (savedUser) {
      ...
      return parsed.role === 'candidate' ? 'upload' : 'dashboard';
    }
    return 'dashboard';
  });
  ...
  const handleLogin = (userData) => {
    ...
    setActiveTab(userData.role === 'candidate' ? 'upload' : 'dashboard');
  };
  ...
  const handleAnalysisComplete = (newReport) => {
    ...
    if (newReport) {
      ...
      setActiveTab(user.role === 'candidate' ? 'upload' : 'evaluations');
    }
  };
  ...
  if (!user) {
    return <Login onLogin={handleLogin} API_URL={API_URL} />;
  }
    ...
    <div className="app-container">
      ...
      <header className="app-header">
        ...
        <nav className={`nav-tabs ${isMenuOpen ? 'open' : ''}`}>
          ...
          <span className="user-role-badge" style={{ marginLeft: '1rem' }}>
